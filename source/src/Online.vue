<template>
  <div id="mi-dashboard-online" class="mi-dashboard">
    <h2>Who has been viewing content?</h2>
    ...
    <template v-if="onlineUsers.length > 0">
      <table class="tg">
        ...
        <tbody>
          <tr class="tg-results" v-for="(item, index) in onlineUsers" :key="index">
            <td>{{ item.name }}</td>
            <td>{{ item.mail }}</td>
          </tr>
        </tbody>
    ...
    <template v-else>
      <table class="tg">
        ...
        <tbody>
          <tr class="tg-results">
            <td>No users have viewed content.</td>
          </tr>
        </tbody>
      </table>
    </template>
...
export default {
  ...
  created() {
    ...
    this.$store.dispatch('loadOnline', {
  ...
  computed: {
    isLoading() {
      return this.$store.getters.onlineLoading;
    },
    onlineUsers() {
      return this.$store.getters.onlineData;
    }
  },
  watch: {}
}
