<template>
  <div id="mi-dashboard-failedsearches" class="mi-dashboard">
    <h2>Number of searches / failed searches</h2>
    ...
    <template v-if="fs().length > 0">
      <table class="tg">
        ...
        <tbody>
          <tr class="tg-results" v-for="(item, index) in fs()" :key="index">
            <td>{{ item.role }}</td>
            <td>{{ item.totalsearches }}</td>
            <td>{{ Math.round((100 / item.totalsearches) * item.success) }}</td>
            <td>{{ Math.round((100 / item.totalsearches) * item.fail) }}</td>
          </tr>
        </tbody>
      </table>
    </template>
...
export default {
  ...
  created() {
    ...
    this.$store.dispatch('loadFailedSearches', {
  ...
  methods: {
    fs() {
      let data = this.$store.getters.failedSearches;
      if (data.length > 0) {
        return data.slice().sort(this.compareTotalSearches); 
      } else {
        return data;
      }
    },
    compareTotalSearches(a, b) {
  ...
  computed: {
    isLoading() {
      return this.$store.getters.failedSearchesLoading;
    },
  },
  watch: {}
}
